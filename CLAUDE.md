...
- Visual world (operator decision after two rejected directions): a Quantopian-platform backtest-report console — red #C50000 navbar, white report pages, nine-statistic strip, algorithm-vs-benchmark chart, tabs, dark log console (see DESIGN.md — measured values, not memory). The amber DEMO MODE bar stays under the navbar on every page while live trading is off. Performance statistics come from `/treasury/metrics` and `/strategies/{code}/metrics` (empyrical); null means not enough history and renders as "—", never an estimate.
